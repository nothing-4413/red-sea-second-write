...
namespace RedSea.Match3.Presentation
{
    public sealed class Match3DemoController : MonoBehaviour
    {
        ...
        private BoardModel board; private TurnResolver resolver; private readonly TurnStateMachine stateMachine = new TurnStateMachine(); private CellPos? selected; private int turnId; private int areaTools; private string toast = "点击两个相邻糖果开始交换"; private ResolveSummary lastSummary;
        private void Start() { if (boardView == null) boardView = FindObjectOfType<BoardView>(); stateMachine.Reset(); var config = levelConfig == null ? new LevelConfig() : levelConfig.ToCore(); board = new BoardModel(config); resolver = new TurnResolver(board); areaTools = config.AreaToolCount; boardView.Bind(board); stateMachine.StateChanged += state => Debug.Log("[Turn] " + state); }
        private void OnGUI()
        {
        ...
        private void UseAreaTool() { if (areaTools <= 0 || stateMachine.InputLocked) { toast = "道具不可用"; return; } var center = selected ?? new CellPos(board.Config.Rows / 2, board.Config.Columns / 2); stateMachine.Enter(GameState.Resolving); var result = resolver.SubmitAreaTool(center, ++turnId); if (!result.IsValid) { toast = "道具目标无效"; return; } areaTools--; lastSummary = result.Summary; stateMachine.BeginAnimation(); StartCoroutine(ConsumeEvents()); }
