...
namespace RedSea.Match3.Presentation
{
    public sealed class Match3DemoController : MonoBehaviour
    {
        ...
        private void OnEventsCompleted() { stateMachine.BeginRefill(); stateMachine.CheckChain(); var endState = resolver.EvaluateEndState(); if (endState == GameState.Win || endState == GameState.Lose) stateMachine.Finish(endState); else stateMachine.ReturnToIdle(); SaveProgress(); LogPerformance(); toast = "棋盘稳定，输入已解锁"; }
