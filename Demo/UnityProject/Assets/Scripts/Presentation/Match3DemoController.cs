...
namespace RedSea.Match3.Presentation
{
    public sealed class Match3DemoController : MonoBehaviour
    {
        ...
        private void OnEventsTimedOut(ErrorSnapshot error) { var endState = resolver.EvaluateEndState(); if (endState == GameState.Win || endState == GameState.Lose) stateMachine.Finish(endState); else stateMachine.ReturnToIdle(); if (ErrorSnapshotExporter.TryExport(error, Application.persistentDataPath, out var path, out var exportError)) { Debug.LogError("[Presentation] " + error.Message + " turn=" + error.TurnId + " snapshot=" + path); toast = "表现播放超时，快照已导出：" + path; } else { Debug.LogError("[Presentation] Snapshot export failed: " + exportError + " turn=" + error.TurnId + " board=" + error.BoardSnapshot); toast = "表现播放超时，快照导出失败；已同步逻辑棋盘"; } }
