...
namespace RedSea.Match3.Presentation
{
    ...
    {
        public LevelConfigAsset levelConfig; public BoardView boardView; public EffectPlayer effectPlayer; public ResultPanel resultPanel;
        private const string DefaultLevelId = "LevelConfig_001"; private const string SaveConfigVersion = "1.0";
        private BoardModel board; private TurnResolver resolver; private readonly TurnStateMachine stateMachine = new TurnStateMachine(); private InputController inputController; private readonly ResultPanelModel resultModel = new ResultPanelModel(); private string toast = "点击两个相邻糖果开始交换"; private ResolveSummary lastSummary; private TurnPerformanceTrace currentPerformance; private Font displayFont;
        private void Awake() { if (boardView == null) boardView = FindObjectOfType<BoardView>(); if (effectPlayer == null) effectPlayer = FindObjectOfType<EffectPlayer>(); if (effectPlayer == null) effectPlayer = new GameObject("EffectPlayer").AddComponent<EffectPlayer>(); if (resultPanel == null) resultPanel = FindObjectOfType<ResultPanel>(); if (resultPanel == null) resultPanel = new GameObject("ResultPanel").AddComponent<ResultPanel>(); resultPanel.Bind(resultModel); displayFont = Resources.Load<Font>("Fonts/display"); stateMachine.StateChanged += state => { resultModel.Update(board, state); Debug.Log("[Turn] " + state); }; Initialize(); }
        private void Initialize() { stateMachine.Reset(); var config = levelConfig == null ? new LevelConfig() : levelConfig.ToCore(); board = new BoardModel(config); resolver = new TurnResolver(board); inputController = new InputController(board, resolver, stateMachine); LoadProgress(); boardView.Bind(board); }
        private void OnGUI()
        {
            ...
            if (GUI.Button(new Rect(700, 205, 180, 48), stateMachine.IsPaused ? "继续" : "暂停")) TogglePause();
            boardView.Draw(inputController?.Selected); HandleInput(); resultPanel.Draw();
        ...
        private void OnEventsCompleted() { stateMachine.BeginRefill(); stateMachine.CheckChain(); var endState = resolver.EvaluateEndState(); if (endState == GameState.Win || endState == GameState.Lose) stateMachine.Finish(endState); else stateMachine.ReturnToIdle(); resultModel.Update(board, stateMachine.State); SaveProgress(); LogPerformance(); toast = "棋盘稳定，输入已解锁"; }
        ...
        private void TogglePause() { if (stateMachine.IsPaused) { stateMachine.Resume(); effectPlayer.SetPaused(false); toast = "已继续"; } else if (stateMachine.Pause()) { effectPlayer.SetPaused(true); toast = "已暂停，输入与表现计时已冻结"; } }
        private string SavePath() { return Path.Combine(Application.persistentDataPath, LocalSaveService.DirectoryName, DefaultLevelId + ".json"); }
        private void LoadProgress() { var result = LocalSaveService.Load(SavePath(), DefaultLevelId, SaveConfigVersion, board.Config.Moves, board.Config.AreaToolCount); var data = result.Data; board.MovesRemaining = data.MovesRemaining; board.AreaToolsRemaining = data.AreaToolsRemaining; board.Score = data.Score; board.ClearedByColor[board.Config.GoalColor] = data.GoalProgress; if (!result.Success) Debug.LogError("[Save] " + result.Message); else if (!result.UsedDefault) toast = "已读取本地进度"; }
        private void SaveProgress() { var data = LocalSaveService.Default(DefaultLevelId, SaveConfigVersion, board.Config.Moves, board.Config.AreaToolCount); data.MovesRemaining = board.MovesRemaining; data.AreaToolsRemaining = board.AreaToolsRemaining; data.Score = board.Score; data.GoalProgress = board.ClearedByColor[board.Config.GoalColor]; var result = LocalSaveService.Save(SavePath(), data); if (!result.Success) Debug.LogError("[Save] " + result.Message); }
        private int GoalProgress() { return board == null ? 0 : board.ClearedByColor[board.Config.GoalColor]; }
        private void Restart() { StopAllCoroutines(); if (effectPlayer != null) effectPlayer.StopPlayback(); resultModel.Hide(); lastSummary = null; currentPerformance = null; Initialize(); toast = "已重开，固定 seed=" + board.Config.Seed; }
