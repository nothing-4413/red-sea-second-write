...
namespace RedSea.Match3.Core.Rules
{
    ...
    {
        public static void Apply(BoardModel board, IList<ResolveEvent> events, int turnId, TurnPerformanceTrace performance = null)
        {
            ...
            finally { if (performance != null) performance.RecordGravityRefill(started); }
        }
    ...
    public static class MvpRulePipeline
    {
        ...
        {
            var performance = new TurnPerformanceTrace(); var started = System.Diagnostics.Stopwatch.GetTimestamp();
            if (!SwapValidator.CanSwap(board, from, to, performance)) return Complete(Invalid(board, turnId), performance, started);
            ...
            return Complete(ResolveUntilStable(board, turnId, events, before, scoreBefore, null, 1, performance), performance, started);
        }

        public static ResolveResult ResolveAreaTool(BoardModel board, CellPos center, int turnId)
        {
            var performance = new TurnPerformanceTrace(); var started = System.Diagnostics.Stopwatch.GetTimestamp();
            if (board.AreaToolsRemaining <= 0 || !board.Config.IsInBounds(center)) return Complete(Invalid(board, turnId), performance, started);
            ...
            if (pieceCells.Count == 0 && obstacleCells.Count == 0) return Complete(Invalid(board, turnId), performance, started);
            ...
            var destroyed = 0; ClearLayer(board, pieceCells, obstacleCells, events, turnId, 1, ref destroyed, performance); GravitySystem.Apply(board, events, turnId, performance);
            return Complete(ResolveUntilStable(board, turnId, events, before, scoreBefore, destroyed, 2, performance), performance, started);
        }

        private static ResolveResult ResolveUntilStable(BoardModel board, int turnId, List<ResolveEvent> events, BoardState before, int scoreBefore, int? destroyedInitial, int nextDepth, TurnPerformanceTrace performance)
        {
            var depth = nextDepth; var destroyed = destroyedInitial ?? 0; var clearLayers = nextDepth - 1;
            while (true)
            {
                var matches = MatchFinder.Find(board, performance); if (matches.Count == 0) break;
                ...
                ClearLayer(board, matches, AdjacentObstacles(board, matches), events, turnId, depth, ref destroyed, performance); GravitySystem.Apply(board, events, turnId, performance); clearLayers++; depth++;
            ...
            {
                var shuffleStarted = System.Diagnostics.Stopwatch.GetTimestamp();
                try { ShuffleSystem.TryShuffle(board, events, turnId); }
                finally { performance.RecordShuffle(shuffleStarted); }
            }
            if (events.Count > board.Config.MaxEvents) { before.Restore(board); throw new InvalidOperationException("Rule error: event queue limit exceeded."); }
        ...
        private static void ClearLayer(BoardModel board, IEnumerable<CellPos> pieceCells, IEnumerable<CellPos> obstacleCells, IList<ResolveEvent> events, int turnId, int depth, ref int destroyed, TurnPerformanceTrace performance)
        {
            ...
            finally { performance.RecordResolve(started); }
        }

        private static ResolveResult Complete(ResolveResult result, TurnPerformanceTrace performance, long started)
        {
            performance.CompleteLogic(started); result.Performance = performance; return result;
        }
