...
namespace RedSea.Match3.Architecture
{
    public sealed class TurnResolver
    {
        ...
        private BoardState lastStableState;
        ...
        public TurnResolver(BoardModel board, IRulePipeline rules = null)
        {
            ...
            lastStableState = BoardState.Capture(board);
        }
        ...
        public ResolveResult SubmitSwap(CellPos from, CellPos to, int turnId)
        {
            ...
            try
            {
                EnsureBoardIntegrity();
                var result = Rules.ResolveSwap(Board, from, to, turnId);
                EnsureBoardIntegrity();
                if (!result.IsValid) return result;
                Events.EnqueueRange(result.Events); Replay.Inputs.Add(new ReplayInput(from, to)); Replay.Summaries.Add(result.Summary); CommitStableState(); return result;
            }
        ...
        public ResolveResult SubmitAreaTool(CellPos center, int turnId)
        {
            ...
            try
            {
                ...
                EnsureBoardIntegrity();
                if (!result.IsValid) return result;
                Events.EnqueueRange(result.Events); Replay.Inputs.Add(ReplayInput.AreaTool(center)); Replay.Summaries.Add(result.Summary); CommitStableState(); return result;
            }
            catch (System.Exception exception)
            {
                return RecoverFromRuleError(before, exception, turnId);
        ...
        private ResolveResult RecoverFromRuleError(BoardState before, System.Exception exception, int turnId)
        {
            ...
            if (!BoardIntegrityValidator.Validate(Board).IsValid) lastStableState.Restore(Board);
            Events.Clear();
        ...
        private void EnsureBoardIntegrity()
        {
            var report = BoardIntegrityValidator.Validate(Board);
            if (!report.IsValid) throw new System.InvalidOperationException("Board integrity violation: " + report.Describe());
        }

        private void CommitStableState() { lastStableState = BoardState.Capture(Board); }
